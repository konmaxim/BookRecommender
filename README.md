...
`bookrec/pipeline/idea_clustering.py` строит **канонический словарь идей** через agglomerative clustering с complete linkage и порогом cosine distance = 0.15. Каждой канонической идее присваивается:

- **centroid** — среднее по эмбеддингам участников кластера, L2-нормированное; используется для матчинга по cos.
- **medoid_text** — фраза, ближайшая к центроиду.
- **canonical_idea_id** — целочисленный id.

Полученный словарь — ~9,000 канонических идей, из которых ~3,150 встречаются в ≥2 книгах.

Стенс и сила хранятся на ребре `(book, idea)`, а не на узле канонической идеи. Одна и та же идея может иметь `+1` в одной книге и `−1` в другой.

### Профиль пользователя по идеям

Из лайкнутой библиотеки `L` строится профиль:
...
- `engagement` — сырая суммарная сила вовлечённости.
- `net_stance` — strength-взвешенное среднее **только по книгам, где пользователь занял позицию**.
...
2. **Стенс.** Стенс и сила читаются с **уже выбранного** кандидата (`s_c`, `σ_c`). argmax считается **только по cos**.

3. **Контрибуция.**:
   - Обе стороны направлены (`p_i ≠ 0` и `s_c ≠ 0`) → stance-ветка. База: `a · sim · σ_c · (s_c · p_i)`. При несогласии (`base < 0`) применяется штраф `λ > 1`.
   - Иначе →: `β · a · sim · σ_c`, всегда ≥ 0.

4. **Агрегация.** Чистая сумма контрибуций по всем идеям профиля
